...
contract ReInvestPool is Ownable {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        uint256 rewardLp = 0;
        if(_amount > 0) {
            kswap.withdraw(pool.pid, _amount); // 提出本金和全部利息
            calcProfit(_pid,pool,false); // 计算利息 有多少
            // 计算当前用户应得利息
    ...
    function calcProfit(uint256 pid,PoolInfo memory pool,bool autoi) private{
        ...
        if (t0.mul(1e10).div(t1)>token0Ba.mul(1e10).div(token1Ba)){
            ...
            (,,liqui) = router.addLiquidity(token0, token1, token0Ba, out, 1, 1, address(this), now.add(1800));
            // 多余的下次一起复投
            if(autoi){
            ...
            (,,liqui) = router.addLiquidity(token0, token1, out, token1Ba, 1, 1, address(this), now.add(1800));
            // 多余的下次一起复投
            if(autoi){
    ...
    function harvest(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        kswap.withdraw(pool.pid, 0); // 提取利息
